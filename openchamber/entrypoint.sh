...
echo "╚══════════════════════════════════════════════╝"
echo ""
echo "[*] OpenChamber version: $(openchamber --version 2>&1 | head -1)" >&2
...
# ===============================================
# Adjust openchamber user's UID/GID
# ===============================================

CURRENT_UID=$(id -u openchamber)
CURRENT_GID=$(getent group openchamber | cut -d: -f3)

if [ "${CURRENT_UID}" != "${TARGET_UID}" ]; then
    echo "[*] Adjusting openchamber user UID: ${CURRENT_UID} -> ${TARGET_UID}" >&2
    usermod -u "${TARGET_UID}" openchamber
fi
if [ "${CURRENT_GID}" != "${TARGET_GID}" ]; then
    echo "[*] Adjusting openchamber group GID: ${CURRENT_GID} -> ${TARGET_GID}" >&2
    groupmod -g "${TARGET_GID}" openchamber
    usermod -g "${TARGET_GID}" openchamber
fi
...
chown -R "${TARGET_UID}:${TARGET_GID}" /home/openchamber 2>/dev/null || true
chown "${TARGET_UID}:${TARGET_GID}" /workspace 2>/dev/null || true

# Create symlink to workspace in home directory
ln -sf /workspace "/home/openchamber/workspace" 2>/dev/null || true

# ===============================================
# Execute command as openchamber user
# ===============================================
